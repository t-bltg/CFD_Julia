...
jacobian(nx, ny, Δx, Δy, wf, k₂, ε=1e-6) = begin
  ...
  j₁f_p = zeros(ComplexF64, nxe, nye)
  j₂f_p, j₃f_p, j₄f_p = (zero(j₁f_p) for _ ∈ 1:3)

  qx = nxe - nx / 2 + 1
  qy = nye - ny / 2 + 1

  @views begin
    j₁f_p[1:nx÷2, 1:ny÷2] = j₁f[1:nx÷2, 1:ny÷2]
    j₁f_p[Int(qx):nxe, 1:ny÷2] = j₁f[Int(nx/2+1):nx, 1:ny÷2]
    j₁f_p[1:nx÷2, Int(qy):nye] = j₁f[1:nx÷2, Int(ny/2+1):ny]
    j₁f_p[Int(qx):nxe, Int(qy):nye] = j₁f[Int(nx/2+1):nx, Int(ny/2+1):ny]
  ...
  j₁ = real(ifft(j₁f_p * (nxe * nye) / (nx * ny)))
  j₂ = real(ifft(j₂f_p * (nxe * nye) / (nx * ny)))
  j₃ = real(ifft(j₃f_p * (nxe * nye) / (nx * ny)))
  j₄ = real(ifft(j₄f_p * (nxe * nye) / (nx * ny)))
  jacp = zeros(Float64, nxe, nye)

  @unroll jacp[1:nxe, 1:nye] = (
    j₁[1:nxe, 1:nye] * j₂[1:nxe, 1:nye] -
    j₃[1:nxe, 1:nye] * j₄[1:nxe, 1:nye]
  )
  ...
  @views begin
    jf[1:nx÷2, 1:ny÷2] = jacpf[1:nx÷2, 1:ny÷2]
    jf[Int(nx/2+1):nx, 1:ny÷2] = jacpf[Int(qx):nxe, 1:ny÷2]
    jf[1:nx÷2, Int(ny/2+1):ny] = jacpf[1:nx÷2, Int(qy):nye]
    jf[Int(nx/2+1):nx, Int(ny/2+1):ny] = jacpf[Int(qx):nxe, Int(qy):nye]
  end

  return jf * (nx * ny) / (nxe * nye)
end
