...
for i ∈ 1:nx for j ∈ 1:ns + 1
  up[i, j] = (gamma - 1.) * (ue[i, j] - uv[i, j] * uv[i, j] / ud[i, j])
...
for i ∈ 1:nxF for j ∈ 1:ns + 1
  upF[i, j] = (gamma - 1.) * (ueF[i, j] - uvF[i, j] * uvF[i, j] / udF[i, j])
...
ax1.plot(xd, ud[:, ns+1], lw=2, color="blue", marker="o", markersize=4, label="Low-resolution")
ax1.set_xlabel("\$x\$")
ax1.set_ylabel("\$ρ\$")
...
ax3.plot(xe, ue[:, ns+1], lw=2, color="green", marker="o", markersize=4, label="Low-resolution")
ax3.set_xlabel("\$x\$")
ax3.set_ylabel("\$e\$")
...
ax4.plot(xp, up[:, ns+1], lw=2, color="orange", marker="o", markersize=4, label="Low-resolution")
ax4.set_xlabel("\$x\$")
ax4.set_ylabel("\$p\$")
