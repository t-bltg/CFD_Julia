...
final_field = readdlm("field_final.txt")
...
cs = ax1.contourf(xx, yy, transpose(wn_t0), levels=80, cmap="YlGnBu", interpolation="bilinear")

ax1.set_title("\$ t=0\$")
# plt[:subplot](ax1); im

for c ∈ cs.collections
  c.set_edgecolor("face")
  c.set_linewidth(.000000000001)
end

cs = ax2.contourf(xx, yy, transpose(wn_t10), levels=80, cmap="YlGnBu", interpolation="bilinear")
ax2.set_title("\$ t=10\$")
# plt[:subplot](ax2); cs
...
cs = ax3.contourf(xx, yy, transpose(wn_t20), levels=80, cmap="YlGnBu", interpolation="bilinear")
ax3.set_title("\$ t=20\$")
# plt[:subplot](ax3); cs
