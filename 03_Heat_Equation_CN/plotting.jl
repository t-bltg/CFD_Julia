...
p1 = plot(
  x, u, line=(6, styles), color=color,
  xlabel="\$X\$", xlims=(minimum(x), maximum(x)),
  ylabel="\$U\$",
  grid=(:none), label=["Exact solution" "Crank-Nicolson scheme"],
  legend=:topright
)
...
p2 = plot(
  x, u_error, line=(3, styles), linestyle=:dot, color=:green,
  xlabel="\$X\$", xlims=(minimum(x), maximum(x)),
  ylabel="\$ϵ\$", grid=(:none),
